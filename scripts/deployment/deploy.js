const {ethers, upgrades, bre} = require("@nomiclabs/buidler");

const {time} = require("@openzeppelin/test-helpers");

const INITIAL_WITHDRAWAL_DELAY = 3600; //seconds
const ERC1820_REGISTRY_ADDRESS = "0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24";
const ERC1820_REGISTRY_DEPLOY_TX =
  "";
const poseidonUnit = require("circomlib/src/poseidon_gencontract");

let hermezKeeperAddress, hermezGovernanceAddress, whiteHackGroupAddress;
var config = require("./deploy_parameters.json");

async function main() {
  // load Mnemonic accounts:
  const signersArray = await ethers.getSigners();

  const [
    deployer,
    hermezKeeperEthers,
    hermezGovernanceEthers,
    whiteHackGroupEthers,
    HEZTokenHolderEthers,
    registryFunder,
    donationEthers,
    bootCoordinatorEthers,
  ] = signersArray;

  // load default account 0 from buidlerEvm
  // Account #0: 0xc783df8a850f42e7f7e57013759c285caa701eb6 (10000 ETH)
  // Private Key: 0xc5e8f61d1ab959b397eecc0a37a6517b8e67a0e7cf1f4bce5591f3ed80199122
  const privateKeyBuidler =
    "0xc5e8f61d1ab959b397eecc0a37a6517b8e67a0e7cf1f4bce5591f3ed80199122";
  const signerDefaultAccount = new ethers.Wallet(
    privateKeyBuidler,
    ethers.provider
  );

  for (let i = 0; i < 8; i++) {
    let tx = {
      to: await signersArray[i].getAddress(),
      value: ethers.utils.parseEther("1.0"),
    };
    await signerDefaultAccount.sendTransaction(tx);
  }

  let chainId = (await ethers.provider.getNetwork()).chainId;
  hermezKeeperAddress =
    config[chainId].hermezKeeperAddress ||
    (await hermezKeeperEthers.getAddress());
  whiteHackGroupAddress =
    config[chainId].whiteHackGroupAddress ||
    (await whiteHackGroupEthers.getAddress());
  hermezGovernanceAddress =
    config[chainId].hermezGovernanceAddress ||
    (await hermezGovernanceEthers.getAddress());
  HEZTokenHolderAddress =
    config[chainId].HEZTokenHolderAddress ||
    (await HEZTokenHolderEthers.getAddress());
  donationAddress =
    config[chainId].donationAddress || (await donationEthers.getAddress());
  bootCoordinatorAddress =
    config[chainId].bootCoordinatorAddress ||
    (await bootCoordinatorEthers.getAddress());

  console.log("hermezKeeperAddress: " + hermezKeeperAddress);
  console.log("whiteHackGroupAddress: " + whiteHackGroupAddress);
  console.log("hermezGovernanceAddress: " + hermezGovernanceAddress);
  console.log("HEZTokenHolderAddress: " + HEZTokenHolderAddress);

  console.log(
    "Deploying contracts with the account:",
    await deployer.getAddress()
  );

  console.log("Account balance:", (await deployer.getBalance()).toString());

  const HermezAuctionProtocol = await ethers.getContractFactory(
    "HermezAuctionProtocol"
  );
  const Hermez = await ethers.getContractFactory("Hermez");
  const WithdrawalDelayer = await ethers.getContractFactory(
    "WithdrawalDelayer"
  );

  // Deploy upgradeable Smartcontracts
  const hermezAuctionProtocol = await upgrades.deployProxy(
    HermezAuctionProtocol,
    [],
    {
      unsafeAllowCustomTypes: true,
      initializer: undefined,
    }
  );
  await hermezAuctionProtocol.deployed();

  // initialize overloaded https://github.com/OpenZeppelin/openzeppelin-upgrades/blob/master/packages/plugin-buidler/test/initializers.js
  const hermez = await upgrades.deployProxy(Hermez, [], {
    unsafeAllowCustomTypes: true,
    initializer: undefined,
  });
  await hermez.deployed();

  console.log("hermez address:", hermez.address);
  console.log("hermezAuctionProtocol address:", hermezAuctionProtocol.address);

  // Deploy withdrawalDelayer Smartcontracts
  let buidlerWithdrawalDelayer = await WithdrawalDelayer.deploy();

  // const withdrawalDelayer = await WithdrawalDelayer.deploy();
  await buidlerWithdrawalDelayer.deployed();

  console.log("withdrawalDelayer address:", buidlerWithdrawalDelayer.address);

  console.log(
    await buidlerWithdrawalDelayer.withdrawalDelayerInitializer(
      INITIAL_WITHDRAWAL_DELAY,
      hermez.address,
      hermezKeeperAddress,
      hermezGovernanceAddress,
      whiteHackGroupAddress
    )
  );

  console.log("withdrawalDelayer initialized");

  if ((await ethers.provider.getCode(ERC1820_REGISTRY_ADDRESS)) == "0x") {
    await registryFunder.sendTransaction({
      to: "0xa990077c3205cbDf861e17Fa532eeB069cE9fF96",
      value: ethers.utils.parseEther("0.08"),
    });
    await ethers.provider.sendTransaction(ERC1820_REGISTRY_DEPLOY_TX);
  }

  const HEZToken = await ethers.getContractFactory("ERC777Mock");
  const buidlerHEZToken = await HEZToken.deploy(
    HEZTokenHolderAddress,
    ethers.BigNumber.from("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"),
    "HEZToken",
    "HEZ",
    []
  );
  await buidlerHEZToken.deployed();
  console.log("HEZToken deployed");

  let genesisBlock = config[chainId].genesisBlock;

  if (genesisBlock == "") {
    genesisBlock = (await ethers.provider.getBlockNumber()) + 100;
  }
  console.log(genesisBlock);

  await hermezAuctionProtocol.hermezAuctionProtocolInitializer(
    buidlerHEZToken.address,
    genesisBlock,
    hermez.address,
    hermezGovernanceAddress,
    donationAddress,
    bootCoordinatorAddress
  );

  console.log("hermezAuctionProtocolInitialized");

  let verifiers = config[chainId].verifiers;
  if (verifiers.length == 0) {
    let VerifierHelper = await ethers.getContractFactory("VerifierHelper");
    let buidlerVerifierHelper = await VerifierHelper.deploy();
    await buidlerVerifierHelper.deployed();
    verifiers = [buidlerVerifierHelper.address];
  }

  let maxTX = config[chainId].maxTX;
  if (maxTX.length == 0) {
    maxTX = [512];
  }

  let poseidonElements = config[chainId].poseidonElements;

  if (poseidonElements.length == 0) {
    let Poseidon2Elements = new ethers.ContractFactory(
      poseidonUnit.abi,
      poseidonUnit.createCode(2),
      deployer
    );

    let Poseidon3Elements = new ethers.ContractFactory(
      poseidonUnit.abi,
      poseidonUnit.createCode(3),
      deployer
    );

    let Poseidon4Elements = new ethers.ContractFactory(
      poseidonUnit.abi,
      poseidonUnit.createCode(4),
      deployer
    );
    const buidlerPoseidon2Elements = await Poseidon2Elements.deploy();
    const buidlerPoseidon3Elements = await Poseidon3Elements.deploy();
    const buidlerPoseidon4Elements = await Poseidon4Elements.deploy();
    await buidlerPoseidon2Elements.deployed();
    await buidlerPoseidon3Elements.deployed();
    await buidlerPoseidon4Elements.deployed();

    poseidonElements = [
      buidlerPoseidon2Elements.address,
      buidlerPoseidon3Elements.address,
      buidlerPoseidon4Elements.address,
    ];
    console.log("buidlerPoseidonElements deployed");
  }

  await hermez.HermezInitializer(
    verifiers,
    maxTX,
    buidlerHEZToken.address,
    hermezGovernanceAddress,
    hermezKeeperAddress,
    hermezAuctionProtocol.address,
    withdrawalDelayer.address,
    poseidonElements[0],
    poseidonElements[1],
    poseidonElements[2],
    config[chainId].feeAddToken,
    config[chainId].forgeL1L2BatchTimeout,
    config[chainId].withdrawalDelay
  );
}

main()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error(error);
    process.exit(1);
  });
