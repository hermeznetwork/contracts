  ...
  describe("utility helpers", function () {
    ...
    it("smt verifier: existence", async () => {
      ...
      for (let i = 1; i < 32; i++) {
        ...
        let siblings = [];
        if (resProof.siblings.length > previousSibilings) {
          ...
          console.log(`smt verifier for ${i} elements, key: ${key}, gas: ${resSm1}, gasDifference: ${resSm1 - previousGasSm} sibilings: ${siblings.length}`);
          previousGasSm = resSm1;
          previousSibilings = siblings.length;
        }
      }
