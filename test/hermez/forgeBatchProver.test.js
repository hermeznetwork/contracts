...
describe("Hermez ERC 20", function () {
  ...
  beforeEach(async function () {
    ...
    const VerifierRollup376_32 = await ethers.getContractFactory(
      "circuit-376-32-256-64_hez1"
    );
    ...
    // deploy helpers
    let buidlerVerifierRollup376_32 = await VerifierRollup376_32.deploy();
    ...
    await buidlerHermez.initializeHermez(
      [buidlerVerifierRollup376_32.address],
  ...
  describe("Forge Batch", function () {
    ...
    it("forge L1-Coordiator-tx Batch ", async function () {
      ...
      const forgerVerifier = new ForgerVerifier(
        maxTx,
        maxL1Tx,
        nLevels,
        buidlerHermez,
      ...
      // forge Batch
      await forgerVerifier.forgeBatch(true, [], l1TxCoordiatorArray);
    ...
    it("expect L1-Tx Queue same as batchbuilder ", async function () {
      ...
      await forgerVerifier.forgeBatch(true, [], []);
    ...
    it("forge L1 user & Coordiator Tx batch", async function () {
      ...
      await forgerVerifier.forgeBatch(true, l1TxUserArray, l1TxCoordiatorArray);
    });
    ...
    it("test instant withdraw circuit", async function () {
      ...
      await forgerVerifier.forgeBatch(true, l1TxUserArray, []);
